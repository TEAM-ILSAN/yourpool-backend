...
def selectRoom(request):
    ...
    # print(userLon)
    people = list(
        ChatRoom.objects.values().filter(lon__range=(userLon0, userLon1)).values()
    )
    # print(people)
    count = len(people)
    # print(count)
...
def outRoom(request):
    ...
    if len(select) >= 1:
        for limit in select.values():
            before = limit["chat_member"]
            bb = {
                v: k for k, v in before.items()
            }  # chat_member에서 req_uid를 찾아서 dict에서 제거
...
def chatList(request):
    ...
    select = (
        ChatRoom.objects.filter(status__in=[1, 2], user=req_uid).values()
        | ChatRoom.objects.filter(
            status__in=[1, 2], chat_member__contains={"1": req_uid}
        ).values()
    )
    print(select)
    if len(select) >= 1:
